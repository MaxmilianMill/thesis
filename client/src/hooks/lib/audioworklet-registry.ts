...
export const createWorketFromSrc = (
  ...
  // The worklet source should be just the class definition
  // We wrap it with registerProcessor
  const blobContent = `${workletSrc}\nregisterProcessor("${workletName}", ${workletSrc.match(/class\s+(\w+)/)?.[1]});`;
