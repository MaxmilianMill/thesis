...
import type { Chat, Message } from "@thesis/types";
...
export class SummaryGenerationService {
    ...
    public async generate(data: IGenerateSummary): Promise<WithStatus<"summary", Summary>> {
        ...
        const prompt = chat?.condition === "control" ? 
            this.buildPersonalizedPrompt(history, userInfo) :
            this.buildStaticPrompt(history);
    ...
    private calculateIndicators(chat: Chat) {

        const taskCompletion = chat.taskList.filter((t) => t.completed).length / chat.taskList.length;
